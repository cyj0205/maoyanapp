...
import { delAdmin, searchAdmins } from "../../service/admins.js"
export default class extends Base {
    render() {
        // console.log(this.$el,'qwe')
        const template = `
        <div>
        <div class="layui-form" lay-filter="search-form" id="search-form" style="display:flex">
        <div>
            <a href="#/admins/addAdmins" class="layui-btn"> <i class="layui-icon">&#xe608;</i>添加管理员</a>
        </div>
        <div class="demoTable">
        ...
        <button class="layui-btn" id="search-btn"  lay-event="search">搜索管理员</button>
      </div>
      </div>
      </div>
      <table class="layui-hide" id="admins-list" lay-filter="admins-list"></table>
           `;
    ...
    afterMount() {
        ...
        this.tableIns = layui.table.render({
            ...
            , cols: [[
                ...
                , { field: 'adminName', title: '账户', width: 300 }
                , { field: 'adminPassword', title: '密码', width: 300, sort: true }
    ...
    handler() {
        const that = this;
        layui.table.on('tool(admins-list)', function (obj) {
            const data = obj.data;
            if (obj.event === 'del') {
                layer.confirm('删除?儿豁', async function (index) {
                    const _id = data._id;
                    const { isDelete } = await delAdmin({ _id });
                    if (isDelete) {
                        layer.alert("删除成功！");
                        obj.del();
                    } else {
                ...
                location.hash = "/admins/updateAdmins";
                //取对应事件的回掉直接调用，非异步。
                setTimeout(() => {
                    layui.form.val('update-form', data);
                    // layui.form.val('add-form', data);
                });
            }
        });
